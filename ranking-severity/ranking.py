###############################################################################
# This script is used to calculate the severity of the outliers and rank them accordingly. It also accepts user input in
# whether a particular outlier is a bug or FP and updates the ranking accordingly.
...
# Feature scores which we calculated separately
feature_scores = {
    'Routing_Policy': 0.10256905808871317,
...
# We iterate all the outlier files and process them separately.
...
for file in files:
    ...
    print(feature_scores[file_name], 'Metric-3!')

    # Calculating the severity score
    df1['Severity_score'] = df1['Metric1'] + df1['Metric2'] + df1['Metric3']
...
# Assigning an outlier_id to each entry. This will be used later in metric 4 when the user has to specify whether
# the outlier is a bug or FP.
